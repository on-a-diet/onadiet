...
The project's public **landing site**, per the meta-standards
[SITE.md](https://github.com/sharvilk/meta-standards/blob/main/SITE.md). Hand-written, self-contained static
...
status badge is truthful (`pre-release` — not on npm yet), and there are no dead links or external requests
...
grow (SITE.md: don't build ahead of reality).

## Deploy

Static — serve the `site/` folder from anywhere (GitHub Pages, Netlify, a bucket). No domain yet; see
CONVENTIONS → _Claiming public names_ before wiring one up.
